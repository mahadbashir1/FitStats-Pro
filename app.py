...
def hypothesis_test():
    b = request.json
    v = b.get("variable")
    mu0 = float(b.get("mu", 0))
    alpha = float(b.get("alpha", 0.05))
    alt = b.get("alternative", "two-sided")  # two-sided, greater, less
    col = df[v].dropna()
    n = len(col)
    x_bar = float(col.mean())
    s = float(col.std())
    se = s / np.sqrt(n)
    t_stat = (x_bar - mu0) / se
    dof = n - 1
    if alt == "two-sided":
        ...
        reject = t_stat < crit
    # Confidence interval
    ci_t = float(stats.t.ppf(1 - alpha / 2, dof))
    ci_lower = x_bar - ci_t * se
    ci_upper = x_bar + ci_t * se
    conclusion = ("Reject H₀. There is sufficient evidence to support the alternative hypothesis."
                  if reject else
                  "Fail to reject H₀. There is not sufficient evidence to support the alternative hypothesis.")
    return jsonify({
        "variable": v, "h0": h0, "h1": h1,
        "n": n, "mean": round(x_bar, 4), "std": round(s, 4), "se": round(se, 4),
        "t_statistic": round(float(t_stat), 4),
        "critical_value": round(float(crit), 4),
        "p_value": round(float(p_val), 6), "alpha": alpha,
        "df": dof, "reject": reject,
        "conclusion": conclusion,
        "confidence_interval": {"lower": round(ci_lower, 4), "upper": round(ci_upper, 4),
                                "level": round((1 - alpha) * 100, 0)},
        "interpretation": f"t({dof}) = {t_stat:.4f}, p = {p_val:.6f}. "
                          f"{'Since p < ' + str(alpha) + ', we reject H₀.' if reject else 'Since p ≥ ' + str(alpha) + ', we fail to reject H₀.'} "
                          f"The {(1 - alpha) * 100:.0f}% CI for the mean is [{ci_lower:.4f}, {ci_upper:.4f}]. "
                          f"{get_context(v)}",
    })


# ── Confidence Intervals ─────────────────────────────────────
@app.route("/api/confidence/mean", methods=["POST"])
def confidence_mean():
    b = request.json
    v = b.get("variable")
    alpha = float(b.get("alpha", 0.05))
    ...
    level = (1 - alpha) * 100
    return jsonify({
        "variable": v, "mean": round(x_bar, 4), "std": round(s, 4),
        "se": round(se, 4), "n": n, "df": dof,
        "t_critical": round(t_crit, 4),
        "ci_lower": round(lower, 4), "ci_upper": round(upper, 4),
        "level": level,
        "interpretation": f"We are {level:.0f}% confident that the true population mean of {v} "
                          f"lies between {lower:.4f} and {upper:.4f}. "
                          f"This means if we repeated this study many times, {level:.0f}% of the intervals constructed would contain the true mean. "
                          f"{get_context(v)}",
    })
